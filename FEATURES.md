...
- ITR:OnPreHitDamage - fires per combat hit (weapon, melee, explosion) after DT/DR and hit location are resolved, before the engine's final dmgMult and hit processing (target, attacker, weapon, damage, hitLocation). Damage is resolved health damage, or resolved limb damage for weapon-condition hits, before dmgMult. Mutable: SetFunctionValue a multiplier (0 negates, 0.5 halves, 2 doubles); multiple handlers multiply together. Scales health, limb and fatigue damage together
