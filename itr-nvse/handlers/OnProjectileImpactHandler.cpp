...
//depth slots are keyed on refID, not elapsed time
static ProjectileLogic::DepthTracker s_depth;

//a live continuation still resolves to a form, a dead one does not - only dead slots are reclaimed
static bool ProjIsLive(UInt32 refID) { return Engine::LookupFormByID(refID) != nullptr; }

//resolve a reserved slot before the vanilla call - Remove() skips reserved slots, so a parent
//slot must be released before the round is killed
struct ReserveGuard {
	int slot;
...
//reverse raycast from the far side back toward the entry, so the first hit is the penetrated
//collider's backface. no hit within kMaxPenetrateThickness means the wall is too thick to pass.
//stacked panels inside the probe window collapse into one backface and traverse as a single hit
...
//emerge a fresh ballistic round on the far side. spawn angles stay 0, the engine re-orients from
...
static char __cdecl HandleImpact(void* proj, int a2, int a3)
{
	...
	//a still-overlapping deflected round re-reports the same ref next frame. static geometry reports
	//refID 0 and the round leaves the surface, so only world refs need the dedup. keep the round
	//alive on a repeat, letting vanilla process it here would kill it
	...
	//resolve feasibility before the pre-event so the event reports the real outcome and the depth
	//slot is reserved where a re-entrant impact cannot steal it. infeasible downgrades to Normal
	bool penReady = false;
	...
	if (outcome == ProjectileLogic::kOutcome_Penetrate) {
		...
		if (chainDepth < kMaxPenetrationDepth && range > 0.0f && base && cell && dl > 1e-6f) {
			...
			//reserve last, once the exit point is known, so no slot is held for a round that cannot pass
			if (FindExitPoint(ctx.pos, penUnit, penExit)) {
				...
				if (penSlot != ProjectileLogic::DepthTracker::kNone) {
					//seed distTravelled so energy = energyOld*falloff, once it falls below
					//minRicochetEnergy Decide returns Normal and the chain stops
	...
	//synchronous because the veto must be known before we act. the reported outcome is the proposed
	//one, a spawn failure can still land as a normal hit. no projectile is passed to handlers, so a
	//listener cannot free it mid-dispatch
	if (s_preProbe.hasListeners && g_eventManagerInterface) {
		static UInt32 s_reentry = 0;
		if (s_reentry >= 8) { //bail to a vanilla hit rather than deflect with the veto ignored
	...
	//emerge the decayed continuation, then let vanilla apply the entry hit and kill the original
...
bool Init(void* nvseInterface)
{
	...
	//4th arg is the proposed outcome, a spawn failure can still land as a normal hit
