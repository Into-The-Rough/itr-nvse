//fixes the engine bug where caution/combat mode never clears (geckwiki engine bugs list)
//combat groups only drop members through code gated on the member being loaded, in combat
//and high-processed, so actors that die or unload outside that window stay grouped forever
//and keep the player flagged in combat (PlayerCharacter+0xDF0/+0xDF1, read by 0x953C50).
//the engine ships the exact cleanup pass (CombatGroup member prune 0x98EB30, drops members
//whose combat controller is null or belongs to another group) but only wires it to savegame
//load. this fix runs that prune whenever combat state changes, stops combat for actors
//demoted out of high process (mirroring the teammate-gated StopCombat in
//MiddleLowProcess::Update 0x90ADAA), and reconciles PlayerCharacter+0x5F8 (bIsDetected),
//whose only vanilla runtime clear depends on which actor happens to sit in high-list slot 0
